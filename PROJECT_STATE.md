...
> **Update 2026-07-16:** MoOS is no longer "ONE look" — it is a **family** built on the
> single UI2 engine. Graphite (dark) + Tidal (light) remain the base; four palette-driven
> members were added — **MoOS Nova, Amethyst, Midnight, Aurora** — each a full package set
> under `org.moos.ui2.*`, generated by `artwork/generate_moos_themes.py` from
> `artwork/moos-themes/palettes.json` (it recolours the working UI2 SVGs/decoration; it does
> NOT revive the retired Nova/UI1 lineages). `moos-theme <name>` switches instantly and
> `moos-apply-theme` preserves the choice. `verify_identity.py` now enforces the *family*
> (all MoOS-branded, no foreign, no old generation). The old top-level `org.moos.nova` /
> `org.moos.ui` are still forbidden. Passages below that say "ONE MoOS look" describe the
> pre-family state. See `FIXES_2026-07-16.md`.
