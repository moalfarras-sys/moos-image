...
يُدمج العمل على الخطة في `main` على دفعات: كل شريحة PR مستقل يُدمج بعد البوابات
السريعة (`just check` والاختبارات المعنية وCI، وبناء صورة محلي عند تغيير ملفات
الإقلاع). الدمج لا يُطلق شيئًا: `build.yml` يدفع وسوم `candidate-*` فقط، و`latest`
لا يتحرك إلا عبر الترقية. في نهاية الدفعة يُشغَّل `scripts/release-candidate.sh`
مرة واحدة: يتوقف الدمج، يُبنى المرشح من `main`، وتعمل إثباتات QCOW2 الثلاثة وISO
وARM بالتوازي على الـdigests الدقيقة، ثم تُطبع الترقية أو تُشغَّل بـ`--promote` عند
نجاح كل إثباتات x86. يعود الدمج بعد الترقية.
