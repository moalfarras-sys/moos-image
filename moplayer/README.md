...
| **الأيقونة والنافذة** | `app_id = org.moos.moplayer` مطابق لملف `.desktop` — بلازما تعرف نافذتها. لا شريط عنوان GNOME: KWin يزخرف النافذة بثيم MoOS UI. |
| **الخط** | IBM Plex Sans + IBM Plex Sans Arabic — خط MoOS UI نفسه، من النظام، بلا تحزيم. |
...
| **التصميم** | رموز MoOS UI (المسافات، الأنصاف، سُلّم الخط) مع هوية MoPlayer الجمرية — انظر [`DESIGN.md`](DESIGN.md). |
...
├── core/         الثيم (Glass Orange Cinema)، اللغة، الإعدادات، الأدوات
...
└── widgets/      مكتبة الودجت (Glass Orange Cinema)
```
