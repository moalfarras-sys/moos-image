...
| ~~Mo Store AppStream long descriptions~~ **Closed 2026-08-02** | curated overlay had erased the real field; the sheet itself existed | Medium | indexer preserves localized bounded AppStream blocks; details renders full plain text | distinct description is shown when present; summary remains fallback | six black-box index tests + focused QML gate | capped at 2048 chars; plain-text only | installed screenshot remains part of visual pass |
...
| ~~Store tab-chain edge cases~~ **Code/gate closed 2026-08-02** | virtual delegates could never form a complete Tab chain | Low | All-apps GridView is one arrow-navigated accessible composite | deterministic category → grid → bulk → rail and reverse traversal | focused QML gate; installed AT-SPI/Tab walk remains | preserves virtualization | built Qt/KDE image for live proof |
