...
| Welcome/Installer full keyboard page-scrolling | store got PageUp/Down + revealFocus; the two wizards' scrollable pages (app picks, timezone list) did not | Medium | `apps/welcome/main.qml`, `apps/installer/main.qml` — port the store's `revealFocus()`/`pageScrollKeys()` pattern | PageUp/Down move every scrollable page; Tab pulls off-screen items into view | headless probe + live Tab-walk with PIL diffs | low — additive pattern | no |
