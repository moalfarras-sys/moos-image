...
2. **Artwork review.** `artwork/` generates 2000+ SVGs. The agent can build an HTML contact sheet
   ...
   `artwork/MOOS_UI2_DESIGN.md` and the visual matrix in
   `docs/DEVELOPMENT_PLAN.md` define the required live verification.
...
- **Figma** — MoOS has no Figma file. The design system is generated by
  `artwork/*.py`; the source of truth is code and
  `artwork/MOOS_UI2_DESIGN.md` is the visual contract.
