...
| --- | --- | --- | --- |
| Station agent | The physical x86 NVIDIA workstation | `feat/moos-one-kde-20260924`: **MoOS One inside KDE** — the MoOS family of native System Settings modules (Overview, Update, What's new, Recovery, Mo PC Remote, Mo AI, Appearance) in their own sidebar group, Mo AI's settings moved into System Settings, Mo AI able to open every settings page and drive KWin through fixed verbs, Meta+Space for Mo AI, one visible Settings entry, and retiring the duplicates the 2026-09-24 audit found (the dormant Command Center, the separate theme window, heroclock, the retired Search package, Discover's unattended updater). | `moos-settings-kcm/**`; `system_files/usr/share/systemsettings/**`; `system_files/usr/bin/moos-settings`, `moos-open`, `moos-control`, `moai-do`, `moai-control`, `moos-theme-picker`, `mo-pc-remote`, `moos-privacy-stop`, `moos-apply-theme`, `moos-selfcheck`; `system_files/usr/libexec/moos-settings-status`, `moai-krunner`, `moos-flatpak-update`; `system_files/usr/lib/moai/moai_tool_schemas.py`; `system_files/usr/share/moos/apps/{settings,moai}/**`, `theme-picker/**`, `whats-new.json`; the `org.moos.*` desktop entries; the MoOS plasmoids; `system_files/usr/share/kwin/**`; the KCM, menu-curation and Discover blocks of `build_files/build.sh` and the KCM block of `build-arm.sh`; the matching tests and `scripts/station/**` |
...
| Oracle agent | The Oracle A1 (aarch64) | `plan/unified-moos-20260924`: P6.7, MoOS reviewed for each new Plasma before the base delivers it (seam registry, 6.8 lock screen, real-greeter gate, Plasma-next canary), plus the MoOS One plan section and rows P2.12/P6.8. PRs #155/#156 are merged. | `build_files/plasma_seams.py`, `build_files/plasma-seams/**`, `scripts/plasma-next/**`, `.github/workflows/plasma-next-canary.yml`, `tests/test_plasma_seams.py`, `tests/test_plasma_next_canary_workflow.py`; the one seam-gate block each in `build_files/build.sh` and `build_files/build-arm.sh`; their lines in the `Justfile` and `tests/repo-gates.sh`. Still mine from earlier: `Containerfile.arm`, `build_files/build-arm.sh`, ARM tests and rows, `build_files/verify_mime_handlers.py`, `tests/test_mime_handlers.py`, the Store architecture files (`catalog.json`, `moos-store-index`, `moos-storectl`, `moos-store`, the Store's `main.qml`, `tests/test_store_arch_honesty.py`) and `system_files/usr/bin/moos-measure-speed`. |
