...
| Oracle agent | The Oracle A1 (aarch64) | ARM boot proofs and the A1's own findings; **now:** the A1 live review of 2026-09-18 (`fix/station-gates-20260918`) — capsule caps in the symbolic icons, one glyph per session action, Search offering installs through a second store | `Containerfile.arm`, ARM tests and ARM rows; `artwork/generate_moos_symbolic_icons.py` and the SVGs and family overlays it and `generate_moos_themes.py` regenerate from it; `tests/test_moos_symbolic_icons.py`; `org.moos.brand/contents/ui/LauncherView.qml`; `etc/xdg/krunnerrc` |
