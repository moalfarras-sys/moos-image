...
| Oracle agent | The Oracle A1 (aarch64) | ARM boot proofs and the A1's own findings. Merged: PR #130 (A1 live review batch), PR #153 (P0.7's cause read out of plymouth's source; the speed probe's dead Klipper channel). **Now:** `fix/arm-app-parity-20260921` — the ARM edition must be able to KEEP the promises `mimeapps.list` makes in every edition. | `Containerfile.arm`, `build_files/build-arm.sh`, ARM tests and ARM rows; `build_files/verify_mime_handlers.py` and `tests/test_mime_handlers.py` (new, mine); the one line each in `build_files/build.sh` and the `Justfile` that calls them; `moai-do`'s `pc_games_supported` and the two setups it guards, `moai-control`'s `arch`, Mo AI's `shownSkillChips`, the skills `graphics-and-nvidia` and `gaming-and-windows-apps`, `tests/test_openclaw_arch_install.py`, `tests/test_moai_app_launch.py`, the stderr flag in `tests/test_moai_rail_layout.py` / `tests/test_moai_agent_loop.py`, `system_files/usr/bin/moos-measure-speed` |
