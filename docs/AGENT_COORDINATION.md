...
| --- | --- | --- | --- |
| Station agent | The physical x86 NVIDIA workstation | **W8 clarity is complete in source** on `feat/glass-clarity-control-20260920`: one Clear/Balanced/Solid authority, event-driven KWin/material bridge, fail-solid reduced transparency and 4K/265% visual review (`THEME_REV` 84). Next here: installed-image three-surface proof, the MoOS mark at three sizes, the specular sweep, and the W7 leftovers | `artwork/generate_moos_design_core.py` and the generated `org/moos/ui` (Tokens, Surface, GlassSurface, Button, Card), `system_files/usr/bin/moos-theme`, `system_files/usr/libexec/moos-material-state`, `system_files/usr/share/moos/theme-picker/**`, `system_files/usr/share/moos/moos-bar.conf` and the layout seed's tray lists, the MoOS plasmoids' glass surfaces (`org.moos.island`, `org.moos.search`), `moplayer/lib/services/system/mpris.dart`, `system_files/etc/xdg/kwinrc`, `system_files/usr/share/kwin/**`, `scripts/station/**`, `tests/qml/motion-review.qml`, `tests/test_station_pointer.py`, `tests/test_moos_local_rpm_authentication.py`, `tests/test_moplayer_mpris_registration.py` |
...
| Oracle agent | The Oracle A1 (aarch64) | ARM boot proofs and A1 findings. PR #155: MIME handlers and native viewer parity; PR #156: Store architecture and engine honesty. Both are green in source/image CI. | `Containerfile.arm`, `build_files/build-arm.sh`, ARM tests and ARM rows; `build_files/verify_mime_handlers.py` and `tests/test_mime_handlers.py` (new, mine); the one line each in `build_files/build.sh` and the `Justfile` that calls them; `moai-do`'s `pc_games_supported` and the two setups it guards, `moai-control`'s `arch`, Mo AI's `shownSkillChips`, the skills `graphics-and-nvidia` and `gaming-and-windows-apps`, `tests/test_openclaw_arch_install.py`, `tests/test_moai_app_launch.py`, the stderr flag in `tests/test_moai_rail_layout.py` / `tests/test_moai_agent_loop.py`, `system_files/usr/bin/moos-measure-speed`; `system_files/usr/share/moos/store/catalog.json`, `system_files/usr/bin/moos-store-index`, `moos-storectl`, `moos-store`, `system_files/usr/share/moos/apps/store/main.qml`, `tests/test_store_arch_honesty.py` |
