...
| --- | --- | --- | --- |
| Station agent | The physical x86 NVIDIA workstation | **W8 — MoOS Aurora Glass** (one material, four depths, a specular edge lit by the wallpaper, one clarity control, the MoOS mark at three sizes). W7 is closed (PR #123); the pointer-driven review tool stays here | `artwork/generate_moos_design_core.py` and the generated `org/moos/ui` (Tokens, Button, Card, Glass surfaces), `artwork/generate_moos_themes.py` glass sections, `system_files/etc/xdg/kwinrc` blur/effects, `system_files/usr/share/moos/theme-picker/**`, the MoOS plasmoids' glass surfaces (`org.moos.island`, `org.moos.search`, `org.moos.ui2.wallpaper`), `scripts/station/**`, `tests/qml/motion-review.qml`, `tests/test_station_pointer.py`, `tests/test_moos_local_rpm_authentication.py`, `system_files/usr/share/kwin/**` |
