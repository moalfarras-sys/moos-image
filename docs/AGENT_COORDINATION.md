...
| --- | --- | --- | --- |
| Station agent | The physical x86 NVIDIA workstation | **W8 is shipped** (Aurora Glass, the Island capsule, the Search button, the status cluster, Search's now-playing row, MoPlayer's two MPRIS defects). Next here: the clarity control's config bridge, the MoOS mark at three sizes, the specular sweep, and the W7 leftovers | `artwork/generate_moos_design_core.py` and the generated `org/moos/ui` (Tokens, Surface, GlassSurface, Button, Card), `system_files/usr/share/moos/moos-bar.conf` and the layout seed's tray lists, the MoOS plasmoids' glass surfaces (`org.moos.island`, `org.moos.search`), `moplayer/lib/services/system/mpris.dart`, `system_files/etc/xdg/kwinrc`, `system_files/usr/share/kwin/**`, `scripts/station/**`, `tests/qml/motion-review.qml`, `tests/test_station_pointer.py`, `tests/test_moos_local_rpm_authentication.py`, `tests/test_moplayer_mpris_registration.py` |
...
| Oracle agent | The Oracle A1 (aarch64) | ARM boot proofs and the A1's own findings; **now:** the A1 live review of 2026-09-18 (`fix/a1-live-review-20260918`) — capsule caps in the symbolic icons, one glyph per session action, Search offering installs through a second store, `THEME_REV` 67 | `Containerfile.arm`, ARM tests and ARM rows; `artwork/generate_moos_symbolic_icons.py` and the SVGs and family overlays it and `generate_moos_themes.py` regenerate from it; `tests/test_moos_symbolic_icons.py`; `org.moos.brand/contents/ui/LauncherView.qml`; `etc/xdg/krunnerrc`; the `THEME_REV` 67 literals; `tests/test_moos_storectl.py` |
