...
- `archive/arm-utm-20260827` held 18 commits that never merged. **Audited
  ...
  readable and writable first. **Deleted from the remote on 2026-09-09** (branch
  hygiene; GitHub now has only `main`). Do not resurrect it without re-auditing —
  cherry-picking from the old tip would be a regression.
