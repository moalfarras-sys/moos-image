...
| First-party apps | Settings, Store, Mo AI, MoPlayer, Remote | MoOS KCM in Plasma System Settings; other apps use `org/moos/ui` on `moos-qml-shell` | native KCM loaded on x86 base in Arabic and English; final-image and installed proof pending | Updater and Recovery transaction sheets are still GTK (P2.1) |
...
| ID | Task | Exit evidence |
| --- | --- | --- |
| P2.1 | **In progress (W9):** put MoOS status, Update, Recovery and Remote inside Plasma System Settings | `kcm_moos` is a native module using the existing read-only status feed and fixed routes to the protected transaction owners. Loaded in Arabic and English on the x86 base. Open: final-image/installed proof, full pending/error/cancel rows and actual transaction lifecycle |
| P2.2 | **In source:** make one Settings window the front door for MoOS and device settings | `moos-settings` now opens Plasma System Settings with the MoOS KCM; display, network, audio, devices, themes and Wayland/KWin remain their real modules in that same host. Hidden Update/Recovery/Remote launchers deep-link to MoOS pages; a per-user fixed-page request reaches the existing single-instance window (proved in an isolated KDE session on 2026-09-24). The duplicate Discover Software Update and generic About this System KCMs are removed on both architectures; Discover's update engine remains. The old Command Center QML is dormant packaged source pending removal after installed proof. Open: AI provider integration and installed-desktop proof |
