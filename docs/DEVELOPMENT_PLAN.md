...
| --- | --- | --- | --- |
| A1 | A Windows program and an Android app really run, from a double-click | **Windows half measured 2026-09-19, and it is half true.** A PE32+ binary executed with the runner's exact prefix and environment (`WINEPREFIX=~/.local/share/moos/windows`, `WINEDLLOVERRIDES=mscoree,mshtml=`) printed `Microsoft Windows 10.0.19045` and exited 0. The same test as a 32-bit PE exits 1 with `err:virtual:map_image_into_view failed to set 60000020 protection on …syswow64\ntdll.dll`. Wine blames `noexec filesystem?` — **that is wrong**: it fails identically in fresh prefixes on tmpfs AND btrfs, so it is neither the filesystem nor the prefix location. **A CORRECTION, recorded because it was published wrong first:** an earlier pass concluded "the image has no 32-bit wine" from `rpm -qa` showing no i686 package, and priced the fix at `wine-core.i686` — 200 packages, ~1 GiB. **That was wrong and the remedy would have been wasted.** Fedora ships new-WoW64, which needs no i686 RPM: it runs i386 PE DLLs on 64-bit Unix libraries. Verified here by parsing the actual PE headers — `i386-windows/ntdll.dll` is a real x86 PE, `wow64.dll` and `wow64cpu.dll` are x86_64, and `x86_64-unix/ntdll.so` is present — so `moos-app-engine`'s `wine_x86_payload()` reports `wow64` on this station. **The 32-bit payload IS present and execution still fails.** The cause remains unestablished: SELinux is Enforcing with `deny_execmem` off, the failure is a memory-protection error on a `syswow64\ntdll.dll` mapping, and confirming it needs the audit log, which needs root this station does not grant passwordlessly. **What this means for the preflight:** it correctly blocks the payload-missing case with a plain bilingual message, but on THIS machine a 32-bit binary resolves `status=available, ready=true`, so the runner proceeds and the owner still gets a silent failure. The defect is narrowed, not closed. | A program opens from a double-click on an INSTALLED image (the station runs `.894`, which predates the runner). A 32-bit program either runs or MoOS says plainly that it cannot. The `.apk` path survives its one-time setup and installs. Journal clean. |
