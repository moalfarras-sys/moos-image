...
| --- | --- | --- | --- |
| A1 | A Windows program and an Android app really run, from a double-click | **Measured 2026-09-19.** A PE32+ program ran with the exact MoOS prefix and printed `Microsoft Windows 10.0.19045`, exit 0. PE32 exits 1 before application code. The earlier diagnosis “no i686 Wine; add ~1 GiB” was wrong: PE headers prove the complete new-WoW64 payload is present. Fresh prefixes on tmpfs and btrfs both fail, and the audit log establishes the immediate cause: SELinux denies `execmod` while mapping the i386 DLL from the composefs backing (`kernel_t` → `lib_t`). Global SELinux relaxation is forbidden. Corrective source now probes MoOS's own built-in PE32 command once per image revision; failure launches no downloaded code and offers optional isolated support. Native launch is supervised and keeps a user-owned diagnostic instead of discarding stderr. APK installation is a Store transaction. | A program opens from a double-click on an INSTALLED corrective image. A 32-bit program either runs or the capability probe routes it safely and plainly. The `.apk` path survives its one-time setup, installs through the Store job, and appears in the launcher. Journal clean. |
