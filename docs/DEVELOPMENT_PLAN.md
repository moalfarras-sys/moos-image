...
| --- | --- | --- | --- |
| A1 | A Windows program and an Android app really run, from a double-click | **WINDOWS: DONE, on the installed image, with pictures.** On `44.20260919.899`, two real PE32+ GUI programs were launched through `moos-run-foreign` — the same path a double-click takes — and appeared as windows on the 4K Arabic desktop: Notepad (`غير معنون - المفكرة`, Arabic menus, right-aligned) and Minesweeper (`الألغام`). Both wore **MoOS's own Aurorae decoration**, and both appeared as running apps **in the MoOS Bar** beside native ones. The resolver answered `برامج ويندوز | Windows programs, ready=true, chosen=wine, needs_setup=false` — no download, which is what `build.sh` always intended. Frames in `test-results/a1-live/`. **LINUX: DONE, full lifecycle through the one authority.** `moos-storectl install com.github.tchx84.Flatseal` returned a real job (`state=success, progress=100`), the app appeared in the launcher as `فلاتسيل`, ran in Arabic RTL, and `moos-storectl remove` took it away cleanly — install, launch and remove all through Mo Store, never `flatpak` directly. **ANDROID: proven to the privilege boundary, not past it.** A real 11.9 MB APK resolves as `تطبيقات أندرويد | Android apps, ready=false, needs_setup=true, route=moos://do/setup-waydroid`; `/dev/binder` exists and `waydroid-container.service` is enabled and active; a double-click routes to `org.moos.runforeign.desktop`; App Drop sniffs it as `android`. Triggering the route opened MoOS's bilingual prompt and **it cancelled itself** — the default-No contract holding in the real product when it cannot ask. The ~1 GB `waydroid init` escalates through `pkexec` and there is no MoOS polkit rule for it, so it needs the owner's password. | The Android half: owner approves the one-time setup, then an APK installs through `moos-storectl` and the app opens. Everything else in this row is now evidence, not a promise. |
