...
Item {
    ...
    // MultiEffect cannot draw the containment with Qt Quick's software backend.
    // Keep the real interactive containment above the edit backdrop instead.
    readonly property bool softwareRendering: GraphicsInfo.api === GraphicsInfo.Software
    Binding {
        target: containmentParent
        property: "z"
        value: 1
        when: editModeItem.softwareRendering && editModeItem.open
    ...
    MultiEffect {
        visible: !editModeItem.softwareRendering
        source: containment
        anchors.fill: parent
        blurEnabled: true
        blurMax: 64
