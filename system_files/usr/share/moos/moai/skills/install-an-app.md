...
---
## Know this first
MoOS has two ways to add software, and neither needs a terminal:
...
4. A `.deb` or `.rpm` package was made for another kind of system and will not install here. Look for the same app in Mo Store first, then for an AppImage from the app's maker.
5. A Windows `.exe` → skill `gaming-and-windows-apps`. An Android `.apk` → `setup_waydroid`. It asks first, needs the administrator password, and downloads about 1 GB the first time.
6. After installing → `open_app` app_id=<id>, and if it does not start → skill `app-wont-start`.
