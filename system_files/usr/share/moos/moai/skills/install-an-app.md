...
4. A `.rpm` package: the last resort, and one you have no tool for. Look for the same app in Mo Store first, then for an AppImage from its maker. If neither exists, the person can choose "Install in MoOS" on the file in the file manager, or use the "Install RPM" button in Mo AI's Apps panel (the file must be in Downloads, Desktop or Documents). Both lead to the same guarded route: MoOS verifies the publisher's signature and refuses an untrusted one, asks first, needs the administrator password, and stages the package into a new system version that appears after a restart (the current version stays for rollback).
