...
store.clear();
assert.equal(h264Failures(), 0, "a new tab starts clean");
assert.equal(h264GivenUp(), false, "a new tab may try H.264 again");
...
assert.equal(h264Failures(), 0, "a throwing storage reads as no history, not as a crash");
assert.equal(h264GivenUp(), false, "private mode must behave exactly as before this existed");
assert.doesNotThrow(() => noteH264Failure(), "recording a failure must never throw");
