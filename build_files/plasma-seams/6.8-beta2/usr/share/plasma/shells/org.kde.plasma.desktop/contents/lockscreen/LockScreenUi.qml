/*
    ...
    PLASMA 6.8 SEAM. This copy is re-derived from plasma-desktop 6.7.91 (6.8
    beta 2) by a three-way merge of MoOS's 6.7 visual additions onto the new
    upstream file: build_files/plasma-seams/README.md says how, and
    build_files/plasma_seams.py installs it only over a 6.8 shell package.
    Upstream beta 2 restores the workspace footer and gates prompts with
    showPrompt. Upstream 6.8 removed
    VirtualKeyboardLoader and the grace-lock timer, and added the authenticator
    model, the PAM-timeout hand-off and a heartbeat — all taken VERBATIM here.
    EVERY authenticator connection, the MainBlock auth path, the StackView and
    the footer are upstream's — this file must never be the reason someone
    cannot unlock their own machine. The additions are purely visual and purely
    MoOS UI2: a bottom graphite scrim, a frosted glass card that fades in with
    the UI, the MoOS emblem + wordmark, the MoOSClock, and a softer entrance.
    All colours come from the active scheme.
*/
