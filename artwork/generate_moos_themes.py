...
def build_icon_theme(key: str, meta: dict) -> None:
    """Generate the palette bridge for MoOS symbolic interface icons.

    The broad application/file vocabulary remains inherited from the proven
    MoOSUI2 dark/light icon themes assembled during the image build.  Only the
    owned symbolic action layer is materialised here, so switching a Global
    Theme updates icon ink, accent, warning and danger together with the rest
    of that palette instead of reusing one teal/light fallback everywhere.
    """
    roles = _roles(key)
...
Directories=moos/actions/scalable
