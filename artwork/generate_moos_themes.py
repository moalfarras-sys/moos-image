...
symbol_icons = importlib.util.module_from_spec(_symbol_spec)
sys.modules[_symbol_spec.name] = symbol_icons
_symbol_spec.loader.exec_module(symbol_icons)

# The first-party application marks. Same reason as the symbolic layer: with
# FollowsColorScheme=false each palette needs its own copy of the marks with
# that palette's inks baked in, or a Global Theme switch would leave the dock
# on the previous palette's teal.
_app_spec = importlib.util.spec_from_file_location(
    "gen_moos_app_icons", ART / "generate_moos_app_icons.py"
)
app_icons = importlib.util.module_from_spec(_app_spec)
sys.modules[_app_spec.name] = app_icons
_app_spec.loader.exec_module(app_icons)
...
def _symbol_accent_ink(roles: dict[str, str]) -> str:
    """Return the nearest accent that stays visible as a standalone glyph.

    A theme primary is sometimes designed as a selected *fill* with paired
    selected_text, not as thin ink on every neutral surface. Daylight's sky
    blue is the concrete case: correct for a filled button, but only 2.70:1 on
    its raised surface. Symbolic facets are non-text UI graphics, so preserve
    the family hue and move only as far toward the already-proven text ink as
    needed to clear WCAG's 3:1 graphical-object threshold everywhere.
    """
    surfaces = tuple(
        roles[name] for name in ("canvas", "surface", "card", "raised")
    )
    candidate = roles["primary"]
    if min(_contrast_ratio(candidate, surface) for surface in surfaces) >= 3:
        return candidate
    for step in range(1, 101):
        candidate = _mix_hex(roles["primary"], roles["text"], step / 100)
        if min(_contrast_ratio(candidate, surface) for surface in surfaces) >= 3:
            return candidate
    raise ValueError("theme text ink itself does not meet symbolic contrast")
...
def lnf_defaults_text(meta: dict) -> str:
    ...
    # while its interface ink/accent/status colours follow this exact palette.
...
def app_mark_masters() -> dict[str, str]:
    """The committed first-party application marks, keyed by icon name.

    Mo AI is excluded on purpose: it is commissioned raster artwork inside an
    SVG wrapper, has no palette roles to bake, and is resolved through the
    inherited base theme.
    """
    local = SHARE / "icons/hicolor/scalable/apps"
    source = local if (local / "moos-store.svg").is_file() else app_icons.SCALABLE
    return {
        path.stem: path.read_text(encoding="utf-8")
        for path in sorted(source.glob("moos-*.svg"))
        if path.stem != "moos-moai"
    }


def build_icon_theme(key: str, meta: dict) -> None:
    """Generate the palette bridge for MoOS's own icons.

    The broad application/file vocabulary remains inherited from the proven
    MoOSUI2 dark/light icon themes assembled during the image build.  What is
    materialised here is what MoOS owns: the symbolic action layer and the
    first-party application marks, each with this palette's inks baked in — so
    switching a Global Theme moves icon ink, accent, warning, danger AND the
    dock's MoOS tiles together instead of leaving them on one teal fallback.
    ...
    (live session captures and executable contrast checks).
    """
    roles = _roles(key)
    target = SHARE / "icons" / meta["style"]
    if target.exists():
        shutil.rmtree(target)
    actions = target / "moos/actions/scalable"
    actions.mkdir(parents=True, exist_ok=True)

    palette = {
        symbol_icons.TEXT: roles["text"],
        symbol_icons.HIGHLIGHT: _symbol_accent_ink(roles),
        symbol_icons.WARNING: roles["warning"],
        symbol_icons.ERROR: roles["negative"],
    }
    for name, symbol in symbol_icons.SYMBOLS.items():
        write(
            actions / f"moos-{name}-symbolic.svg",
            symbol_icons.render(name, symbol, palette),
        )

    apps = target / "moos/apps/scalable"
    apps.mkdir(parents=True, exist_ok=True)
    app_palette = app_icons.icon_roles(roles)
    for name, master in app_mark_masters().items():
        write(apps / f"{name}.svg", app_icons.recoloured(master, app_palette))

    inherited = "MoOSUI2Light" if meta.get("light") else "MoOSUI2"
    write(target / "index.theme", f"""[Icon Theme]
Name={meta['name']} Symbols
Comment=Palette-matched MoOS Tidal Cut interface symbols
Inherits={inherited}
# Normal QIcon rendering can receive the application's QPalette rather than
# the Plasma surface's Kirigami color set. That produced dark symbols on the
# live dark Launcher. Owned QML controls use explicit masks for live accent
# states; this overlay keeps its WCAG-checked palette fallback deterministic.
